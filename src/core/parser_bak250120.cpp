...
void Parser::parsing(std::string payload) {
    ...
    if (topic_type == TopicType::RGB) {
        ...
        fragmentChecker();
        if (fragment_index == FRAGMENT_NUM - 1){
            // std::cout << fragmentChecker() << globalChecker() << !write_locker << std::endl;
            // if (fragmentChecker() && !write_locker){
            if (fragmentChecker() && globalChecker() && !write_locker){
                shm_ptr[0] = 0;
                for (int i = 0; i< 4; i++){
                    shm_ptr[1+i] = payload[i];
                }
                for (int i = 0; i < 3 * WIDTH * HEIGHT; i++){
                    shm_ptr[1+4+i] = payload_tmp[i];
                }
                write_locker = true;
                shm_ptr[0] = 1;
                std::cout << "image" << global_index << std::endl;
            }
            else {
                // std::cout << "fragments are not fullfilled" << std::endl;
            }
    ...
    //DEPTH 카메라
        ...
        if (fragment_index == FRAGMENT_NUM - 1){
            ...
            if (fragmentChecker() && globalChecker() && !write_locker){
                ...
                write_locker = true;
                shm_ptr[0] = 1;
                std::cout << "depth" << global_index << std::endl;
            }
            else {
                // std::cout << "fragments are not fullfilled" << std::endl;
            }
    ...
    // POSE
        ...
        std::memcpy(&global_index, &payload[0], sizeof(uint32_t));
        // std::cout << "curr: " << global_index << std::endl;
        // std::cout << "prev: " << global_index_prev << std::endl;
        if (global_index != global_index_prev) {
            if (globalChecker()){
                shm_ptr[0] = 0;
                global_index_prev = global_index;
                for (int i = 0; i < 4+(3+4)*4; i++) {
                    shm_ptr[1+i] = payload[i];
                }
                shm_ptr[0] = 1;
                std::cout << "pose" << global_index << std::endl;
            }
        }
        // for (int i = 0; i < (3+4)*4; i++) {
        //         shm_ptr[i] = payload[4+i];
        //     }
        // write_locker = true; 
    }
...
bool Parser::globalChecker() {
    ...
    if (topic_type == TopicType::RGB){
        global_check[static_cast<uint32_t>(TopicType::RGB)]= global_index;
    }
    else if (topic_type == TopicType::DEPTH){
        global_check[static_cast<uint32_t>(TopicType::DEPTH)]= global_index;
        // std::cout << "global: " << global_check[0] << " " << global_check[1] << " " << global_check[2] << std::endl;
    }
    else if (topic_type == TopicType::POSE){
        global_check[static_cast<uint32_t>(TopicType::POSE)]= global_index;
    }
